-- -------------------------------------------------------
-- Delete context_data_type
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.d_context_data_type(
  IN id UUID,
...
DECLARE
  ...
  c_service_name TEXT := 'd_context_data_type';
...
BEGIN
  ...
  DELETE FROM pareto.context_data_type 
   WHERE context_data_type.id = v_id
     AND context_data_type.updated_at = v_updated_at;
