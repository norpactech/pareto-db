...
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.u_data_object(
  IN id uuid, 
  IN id_schema uuid, 
  IN name varchar, 
  IN description text, 
  IN has_identifier boolean, 
  IN has_audit boolean, 
  IN has_active boolean, 
...
DECLARE
  ...
  c_service_name TEXT := 'u_data_object';
  ...
  v_has_identifier boolean := has_identifier;
  v_has_audit boolean := has_audit;
  v_has_active boolean := has_active;
...
BEGIN
  ...
  UPDATE pareto.data_object SET
    ...
    has_identifier = v_has_identifier, 
    has_audit = v_has_audit, 
    has_active = v_has_active, 
