...
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.deact_context(
  IN id UUID,
...
DECLARE
  ...
  c_service_name TEXT := 'deact_context';
...
BEGIN
  ...
  UPDATE pareto.context
     SET is_active = FALSE
   WHERE context.id = v_id
     AND context.updated_at = v_updated_at
  RETURNING context.id, context.updated_at INTO v_id, v_updated_at;
...
-- -------------------------------------------------------
-- Reactivate context (Soft Undelete)
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.react_context(
  IN id UUID,
  IN updated_at TIMESTAMPTZ,
  IN updated_by TEXT
)
...
DECLARE
  ...
  c_service_name TEXT := 'react_context';
