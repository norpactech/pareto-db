...
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.deact_schema(
  IN id UUID,
...
DECLARE
  ...
  c_service_name TEXT := 'deact_schema';
...
BEGIN
  ...
  UPDATE pareto.schema
     SET is_active = FALSE
   WHERE schema.id = v_id
     AND schema.updated_at = v_updated_at
  RETURNING schema.id, schema.updated_at INTO v_id, v_updated_at;
...
-- -------------------------------------------------------
-- Reactivate schema (Soft Undelete)
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.react_schema(
  IN id UUID,
  IN updated_at TIMESTAMPTZ,
  IN updated_by TEXT
)
...
DECLARE
  ...
  c_service_name TEXT := 'react_schema';
