...
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.deact_index(
  IN id UUID,
...
BEGIN
  ...
  UPDATE pareto.index
     SET is_active = FALSE
   WHERE index.id = v_id
     AND index.updated_at = v_updated_at
  RETURNING index.id, index.updated_at INTO v_id, v_updated_at;
...
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.react_index(
  IN id UUID,
  IN updated_at TIMESTAMPTZ,
  IN updated_by TEXT
)
