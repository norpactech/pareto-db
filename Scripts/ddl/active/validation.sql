-- -------------------------------------------------------
-- Deactivate validation (Soft Delete)
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.deact_validation(
  IN id UUID,
...
DECLARE
  ...
  c_service_name TEXT := 'deact_validation';
...
BEGIN
  ...
  UPDATE pareto.validation
     SET is_active = FALSE
   WHERE validation.id = v_id
     AND validation.updated_at = v_updated_at
  RETURNING validation.id, validation.updated_at INTO v_id, v_updated_at;
...
-- -------------------------------------------------------
-- Reactivate validation (Soft Undelete)
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.react_validation(
  IN id UUID,
  IN updated_at TIMESTAMPTZ,
  IN updated_by TEXT
)
...
DECLARE
  ...
  c_service_name TEXT := 'react_validation';
