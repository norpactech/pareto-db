-- -------------------------------------------------------
-- Deactivate ref_tables (Soft Delete)
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.deact_ref_tables(
  IN id UUID,
...
DECLARE
  ...
  c_service_name TEXT := 'deact_ref_tables';
...
BEGIN
  ...
  -- ------------------------------------------------------
  -- Deactivate ref_tables
  -- ------------------------------------------------------

  UPDATE pareto.ref_tables
     SET is_active = FALSE
   WHERE ref_tables.id = v_id
     AND ref_tables.updated_at = v_updated_at
  RETURNING ref_tables.id, ref_tables.updated_at INTO v_id, v_updated_at;
...
-- -------------------------------------------------------
-- Reactivate ref_tables (Soft Undelete)
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.react_ref_tables(
  IN id UUID,
  IN updated_at TIMESTAMPTZ,
  IN updated_by TEXT
)
...
DECLARE
  ...
  c_service_name TEXT := 'react_ref_tables';
