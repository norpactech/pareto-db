-- -------------------------------------------------------
-- Deactivate plugin (Soft Delete)
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.deact_plugin(
  IN id UUID,
...
DECLARE
  ...
  c_service_name TEXT := 'deact_plugin';
...
BEGIN
  ...
  UPDATE pareto.plugin
     SET is_active = FALSE
   WHERE plugin.id = v_id
     AND plugin.updated_at = v_updated_at
...
-- -------------------------------------------------------
-- Reactivate plugin (Soft Undelete)
-- ------------------------------------------------------
CREATE OR REPLACE FUNCTION pareto.react_plugin(
  IN id UUID,
  IN updated_at TIMESTAMPTZ,
  IN updated_by TEXT
)
...
DECLARE
  ...
  c_service_name TEXT := 'react_plugin';
