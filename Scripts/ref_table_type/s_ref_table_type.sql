...
begin
  ...
  if not found then
    ...
    response.id := null;
    response.updated := null;
    response.message := 'Error: Table Type not Found for Primary Key: ' || coalesce(in_id::text, 'NULL');
...
begin
  ...
  if not found then
    ...
    response.id := null;
    response.updated := null;
    response.message := 'Error: Reference Table does not Exist for Primary Key: ' || coalesce(in_id::text, 'NULL');
